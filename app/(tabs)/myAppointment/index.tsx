...
function NotPassedAppointment({passed}: any) {
        ...
        const unsubscribe = firestore()
            ...
            .onSnapshot(async (querySnapshot) => {
                ...
                for (const element of querySnapshot.docs) {
                    ...
                    tempAppointment.push({id: element.id, appointment: data, caregiver, idCaregiver: caregiverSnap.id});
                }
    ...
    const handleCancel = async (id: string, idCaregiver: string, dateTime: any) => {
        ...
        const availabilitySnapshot = await firestore().collection(`Users/${idCaregiver}/Availabilities`).where('date', '==', dateStr).limit(1).get()
                ...
                renderItem={({item}) => (
                    <View style={unique.card}>
                        ...
                        <Text style={unique.address}>
                            {item.caregiver.address.street}{"\n"}
                            {item.caregiver.address.city}{"\n"}
                            {item.caregiver.address.postalCode}{"\n"}
                            {item.caregiver.address.country}
                        </Text>
                        {!passed ? (
                            <Button title={'Annuler'}
                                    onPress={() => handleCancel(item.id, item.idCaregiver, item.appointment.dateTime)}/>
                        ) : null}
                    </View>
                )}
            />
